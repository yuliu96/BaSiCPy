...
class ApproximateFit(BaseFit):
    ...
    def _step(
        ...
        S_hat = jax.scipy.fft.dctn(temp_W.reshape(mm, nn), norm = "ortho", type = 2)
        S_hat = _jshrinkage(S_hat, self.smoothness_flatfield / (self._ent1 * mu))
        S = jax.scipy.fft.idctn(S_hat, norm = "ortho", type = 2)
        ...
        if self.get_darkfield:
            ...
            # smooth A_offset
            D_R = jax.scipy.fft.dctn(D_R, norm = "ortho", type = 2)
            D_R = _jshrinkage(D_R, self.smoothness_darkfield / (self._ent2 * mu))
            D_R = jax.scipy.fft.idctn(D_R, norm = "ortho", type = 2)
            D_R = _jshrinkage(D_R, self.smoothness_darkfield / (self._ent2 * mu))
            D_R = D_R + Z
